...
def ClassToInterval(value):
  if value == "A":
    return "up to 40 days"
  elif value == "B":
    return "from 41 to 80 days"
  elif value == "C":
    return "from 81 to 120 days"
  elif value == "D":
    return "from 121 to 160 days"
  elif value == "E":
    return "over 160 days"
  else:
    return ""
...
def LoadVectorizers():
  ...
  if Path(f"{storage_classifier}/vectorizer_PT.tfidf").is_file(): 
    print(f"Carregando vectorizer da predição do tempo de correção das vulnerabilidades.")
    vectorizerPT = joblib.load(f"{storage_classifier}/vectorizer_PT.tfidf")
  else:
    sys.exit("Vectorizer da predição do tempo de correção das vulnerabilidades não encontrado.")
  
  return vectorizer2, vectorizer3, vectorizerPT
...
def LoadModels():
  ...
  if Path(f"{storage_classifier}/classifier_{versao}.model").is_file():
    print(f'Carregando classificador da predição do tempo de correção das vulnerabilidades.')
    versoes['PT']['classifier'] = joblib.load(f"{storage_classifier}/classifier_{versao}.model")
  else:
    sys.exit(f"Classificador da predição do tempo de correção das vulnerabilidades.")
...
def Predict(text, vectorizer2, vectorizer3, vectorizerPT, models):
  ...
  txt_vectorizedPT, vuln_termsPT = VectorizerText(txt, vectorizerPT)
  for model in models:
    if model == 'PT':
      ret[model] = {}
      features = np.concatenate((np.array([ret['CVSS2']['metricas']['AV']['code_classes'][ret['CVSS2']['metricas']['AV']['index_proba']], ret['CVSS2']['metricas']['AC']['code_classes'][ret['CVSS2']['metricas']['AC']['index_proba']], ret['CVSS2']['metricas']['AU']['code_classes'][ret['CVSS2']['metricas']['AU']['index_proba']], ret['CVSS2']['metricas']['C']['code_classes'][ret['CVSS2']['metricas']['C']['index_proba']], ret['CVSS2']['metricas']['I']['code_classes'][ret['CVSS2']['metricas']['I']['index_proba']], ret['CVSS2']['metricas']['A']['code_classes'][ret['CVSS2']['metricas']['A']['index_proba']]]), np.array(txt_vectorizedPT)))
      y = models[model]['classifier'].predict_proba([features])
      ret[model]['code_classes'] = models[model]['classifier'].classes_
      ret[model]['name_classes'] = []
      for code in ret[model]['code_classes']:
        ret[model]['name_classes'].append(ClassToInterval(code))
      ret[model]['predict_proba'] = y[0]
      ret[model]['index_proba'] = np.where(y[0] == np.max(y[0]))[0][0]
    else:
      ret[model] = {}
      ret[model]['metricas'] = {}
  ...
  for term in vuln_termsPT:
    ...
    if zeros == 0:
      ...
      trigram += 1
  ret['PT']['Statistics'] = {}
  ret['PT']['Statistics']['Terms_Total'] = len(vuln_termsPT)
  ret['PT']['Statistics']['Terms_Processed'] = proc_term
  ret['PT']['Statistics']['Terms_Perc_Processed'] = proc_term / len(vuln_termsPT)

  ret['PT']['Statistics']['Unigram_Total'] = unigram
  ret['PT']['Statistics']['Unigram_Processed'] = proc_unigram
  ret['PT']['Statistics']['Unigram_Perc_Processed'] = proc_unigram / unigram

  ret['PT']['Statistics']['Bigram_Total'] = bigram
  ret['PT']['Statistics']['Bigram_Processed'] = proc_bigram
  ret['PT']['Statistics']['Bigram_Perc_Processed'] = proc_bigram / bigram

  ret['PT']['Statistics']['Trigram_Total'] = trigram
  ret['PT']['Statistics']['Trigram_Processed'] = proc_trigram
  ret['PT']['Statistics']['Trigram_Perc_Processed'] = proc_trigram / trigram
